<%-- Detail screen for Synopsis entity      --%>
...
<link href="<s:url value='/static/css/styles.css' includeParams='none'/>" rel="stylesheet" />
  ...
  <body>
    ...
    <div class="container">
		...
		<s:form name="synopsisForm" action="synopsis" cssClass="well">  
		
			<s:textfield name="current.bookId"      		label="bookId" />
			<s:textfield name="current.synopsis"      		label="synopsis" />
			...
			<td colspan="2">
			 	...
			 	<input type="button" class="btn btn-info" value="Delete" onclick="directDelete('<s:property value="#attr.current.bookId" />')" />
	...
	<script type="text/javascript">
		...
		var urlBase = '<s:url action="synopsis" includeParams="none" />';
		
		function changeActionAndsubmit(url) {
			document.synopsisForm.action = urlBase + '/' + url;
		    document.synopsisForm.submit();
		}
		
		function directDelete(idsynopsis) {
			document.location = urlBase + '/delete/' + idsynopsis;
		}
