...
<link href="<s:url value='/static/css/styles.css' includeParams='none'/>" rel="stylesheet" />
  ...
  <body>
    ...
    <div class="container">
		...
		<s:form name="badgeForm" action="badge" cssClass="well">  
		
			<s:textfield name="current.badgeNumber"      		label="badgeNumber" />
			<s:textfield name="current.authorizationLevel"      		label="authorizationLevel" />
			<s:textfield name="current.endOfValidity"      		label="endOfValidity" />
			...
			<td colspan="2">
			 	...
			 	<input type="button" class="btn btn-info" value="Delete" onclick="directDelete('<s:property value="#attr.current.badgeNumber" />')" />
